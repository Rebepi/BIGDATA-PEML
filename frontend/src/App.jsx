...
function Navbar() {
  ...
  const dash = useDashboard()
  const mobileMenuOpen = dash?.sidebarMobileOpen || false
  const setMobileMenuOpen = dash?.setSidebarMobileOpen || (() => { })
      ...
      <div className="navbar-inner">
        <div className="navbar-left">
          ...
          <div className="navbar-brand">
            <PeruFlag width={38} height={25} />
            <div className="navbar-brand-text">
              <span className="navbar-brand-title">GastoPerú</span>
              <span className="navbar-brand-sep hidden sm:inline">·</span>
              <span className="navbar-brand-subtitle hidden sm:block">Monitor Fiscal</span>
            </div>
          </div>

          {!isLoginPage && isDashboard && (
            <div
              className="navbar-title-wrap hidden 2xl:flex items-center gap-2 pl-3 ml-2"
        ...
        <div className="navbar-right">
          {!isLoginPage && usuario && (
            <>
              {isDashboard && dash && (
                <div className="navbar-dashboard-controls hidden md:flex items-center gap-1.5">
                  <div
                    className="flex 2xl:hidden items-center gap-1.5 px-2 py-1 rounded-xl"
                    style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-main)' }}
                  >
                    <Calendar size={13} style={{ color: 'var(--peru-red)', flexShrink: 0 }} />
                    <span className="text-[10px] font-bold uppercase hidden lg:inline" style={{ color: 'var(--text-muted)' }}>
                      Año:
                    </span>
                    <select
                      value={anio}
                      onChange={(e) => dash.setAnio(Number(e.target.value))}
                      className="select-field text-xs py-0.5 px-1 font-mono font-bold"
                      style={{
                    ...
                    >
                      {anios.map((a) => (
                        <option key={a} value={a} style={{ background: '#121216', color: '#fff' }}>
                          {a}
                        </option>
                      ))}
                    </select>
                  ...
                  <div
                    className="hidden 2xl:flex items-center gap-0.5 p-0.5 rounded-xl"
                    style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-main)' }}
                  >
                    <Calendar size={11} style={{ color: 'var(--text-muted)', marginLeft: 6, marginRight: 2 }} />
                    {anios.map((a) => (
                      <button
                        key={a}
                  ...
                  <button
                    ...
                    className="btn-secondary py-1 px-2 sm:px-2.5 text-xs flex items-center gap-1 rounded-xl"
                  ...
                  >
                    {dash.exportSuccess ? (
                      <>
                        ...
                        <span style={{ color: 'var(--color-success)', fontWeight: 600 }} className="text-xs hidden md:inline">Descargado</span>
                      </>
                    ) : (
                      <>
                        <Download size={13} style={{ color: 'var(--peru-red)' }} />
                        <span className="text-xs hidden md:inline">{dash.exportando ? '...' : 'Exportar'}</span>
                      </>
                    )}
                  </button>
              ...
              <div className="navbar-clock hidden lg:flex">
                <span className="navbar-clock-time">{timeStr}</span>
                <span className="navbar-clock-date">{dateStr}</span>
              </div>

              <div className="navbar-live-indicator hidden sm:flex">
