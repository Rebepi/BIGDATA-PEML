...
import { getMockMensualRegion, getMockSectoresRegion } from '../data/mockData'
...
function PanelDetalle({ region, resumenData, anio, onClose }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setTimeout(() => {
      setMensual(getMockMensualRegion(region.id))
      setSectores(getMockSectoresRegion(region.id))
      setLoading(false)
    }, 300)
  }, [region?.id, anio])
