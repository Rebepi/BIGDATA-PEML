...
import { getModeloMetricas, getModeloMetricasRegiones, getModeloMetricasSectores } from '../services/api'
...
export default function ComparativaModelos() {
  ...
  useEffect(() => {
    getModeloMetricasRegiones()
      .then(setRegiones)
      .catch(() => setRegiones([]))
  }, [])

  useEffect(() => {
    getModeloMetricasSectores(regionId)
      .then(setSectores)
      .catch(() => setSectores([]))
    setSector(null)
  }, [regionId])
  ...
  const cargar = useCallback(() => {
    setLoading(true)
    setError(null)
    getModeloMetricas(regionId, sector, 200)
      .then(setMetricas)
