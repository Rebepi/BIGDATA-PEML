...
import { MOCK_RESUMEN, MOCK_MENSUAL } from '../data/mockData'
...
export default function ComparativaRegiones({ regiones = [], anio = 2024, onClose }) {
  ...
  const [region2Id, setRegion2Id] = useState(regiones[1]?.id || 2)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setTimeout(() => {
      const r1 = MOCK_RESUMEN.find(r => r.region_id === region1Id) || MOCK_RESUMEN[0]
      const r2 = MOCK_RESUMEN.find(r => r.region_id === region2Id) || MOCK_RESUMEN[1]
      const factor1 = (r1.region_id % 5) * 0.1 + 0.6
      const factor2 = (r2.region_id % 5) * 0.1 + 0.6
      const mockData = {
        region1: { region_id: r1.region_id, region_nombre: r1.region_nombre, monto_pim: r1.monto_pim, monto_devengado: r1.monto_devengado, monto_girado: r1.monto_girado, porcentaje_ejecucion: r1.porcentaje_ejecucion },
        region2: { region_id: r2.region_id, region_nombre: r2.region_nombre, monto_pim: r2.monto_pim, monto_devengado: r2.monto_devengado, monto_girado: r2.monto_girado, porcentaje_ejecucion: r2.porcentaje_ejecucion },
        diferencia_porcentaje: Math.abs(r1.porcentaje_ejecucion - r2.porcentaje_ejecucion),
        mensual: MOCK_MENSUAL.map(m => ({
          mes_nombre: m.mes_nombre,
          region1_devengado: m.monto_devengado * factor1 * 0.08,
          region2_devengado: m.monto_devengado * factor2 * 0.08,
          region1_pim: m.monto_pim * factor1 * 0.08,
          region2_pim: m.monto_pim * factor2 * 0.08,
        })),
        sectores: [
          { sector: 'Educacion', region1_devengado: r1.monto_devengado * 0.28, region2_devengado: r2.monto_devengado * 0.28, region1_porcentaje: 87, region2_porcentaje: 83 },
          { sector: 'Salud', region1_devengado: r1.monto_devengado * 0.18, region2_devengado: r2.monto_devengado * 0.18, region1_porcentaje: 84, region2_porcentaje: 79 },
          { sector: 'Transporte', region1_devengado: r1.monto_devengado * 0.14, region2_devengado: r2.monto_devengado * 0.14, region1_porcentaje: 78, region2_porcentaje: 72 },
          { sector: 'Interior', region1_devengado: r1.monto_devengado * 0.10, region2_devengado: r2.monto_devengado * 0.10, region1_porcentaje: 83, region2_porcentaje: 80 },
          { sector: 'Agricultura', region1_devengado: r1.monto_devengado * 0.08, region2_devengado: r2.monto_devengado * 0.08, region1_porcentaje: 74, region2_porcentaje: 71 },
        ],
      }
      setData(mockData)
      setLoading(false)
