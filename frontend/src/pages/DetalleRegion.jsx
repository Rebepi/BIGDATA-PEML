...
import { getResumen, getEvolucionMensual, getSectores, getAniosDisponibles, exportarGastoCsv, getPrediccion } from '../services/api'
...
export default function DetalleRegion() {
  ...
  const [sectorPrediccion, setSectorPrediccion] = useState(null)

  useEffect(() => {
    getAniosDisponibles().then((res) => {
      if (res?.length > 0) setAnios(res)
    }).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      setError(null)
      try {
        const [resData, menData, secData, predData] = await Promise.all([
          getResumen(anio).catch(() => []),
          getEvolucionMensual(anio, parseInt(regionId, 10)).catch(() => []),
          getSectores(anio, parseInt(regionId, 10), 12).catch(() => []),
          getPrediccion(parseInt(regionId, 10), anio, 3, sectorPrediccion).catch(() => null),
        ])

        const found = resData.find((r) => r.region_id === parseInt(regionId, 10))
        setRegionData(found || null)
        setMensual(menData)
        setSectores(secData)
        setPrediccion(predData)
      } catch (err) {
        setError(err.message || 'Error al cargar los datos de la región.')
      } finally {
        setLoading(false)
      }
    }
    cargar()
  }, [regionId, anio, sectorPrediccion])
  ...
  const handleExport = async () => {
    ...
    try {
      await exportarGastoCsv(anio, parseInt(regionId, 10))
