...
import {
  getKPIs, getResumen, getRanking, getRegiones, getAlertas,
  getEvolucionMensual, getSectores, getNiveles, getAniosDisponibles,
  getPrediccion
} from '../services/api'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    getAniosDisponibles().then(res => { if (res?.length > 0) setAnios(res) }).catch(() => {})
  }, [setAnios])

  const cargarAlertas = async () => {
    try {
      const altData = await getAlertas()
      setAlertas(altData || [])
      setAlertCount((altData || []).filter(a => !a.leida).length)
    } catch {}
  }
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      setError(null)
      try {
        const [kpiData, resData, rankData, regData, altData, menData, secData, nivData, predData] = await Promise.all([
          getKPIs(anio).catch(() => null),
          getResumen(anio).catch(() => []),
          getRanking(anio).catch(() => []),
          getRegiones().catch(() => []),
          getAlertas().catch(() => []),
          getEvolucionMensual(anio).catch(() => []),
          getSectores(anio, null, 8).catch(() => []),
          getNiveles(anio).catch(() => []),
          getPrediccion(0, anio, 3).catch(() => null),
        ])
        setKpis(kpiData); setResumen(resData); setRanking(rankData)
        setRegiones(regData); setAlertas(altData); setMensual(menData)
        setSectores(secData); setNiveles(nivData); setPrediccion(predData)
        setAlertCount((altData || []).filter(a => !a.leida).length)
      } catch (err) {
        setError(err.message || 'Error al cargar datos del MEF.')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div
      ...
      style={{ '--sidebar-w': sidebarCollapsed ? '68px' : '244px' }}
    >
      <Sidebar />
      ...
      <div className="dashboard-content animate-fade-in">
        {error && (
          <div
            ...
            style={{ padding: '0.75rem 1rem', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', fontSize: '0.8rem', background: 'rgba(200,0,10,0.07)', border: '1px solid rgba(200,0,10,0.25)', marginBottom: '0.65rem', flexShrink: 0 }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <AlertTriangle size={15} style={{ color: 'var(--peru-red)', flexShrink: 0 }} />
              <span style={{ color: 'var(--color-error-text)' }}>{error}</span>
            </div>
            <button onClick={() => dash?.setAnio(a => a)} className="btn-ghost" style={{ fontSize: '0.75rem', color: 'var(--peru-red)' }}>
              Reintentar
            </button>
          </div>
        )}
        ...
        <ErrorBoundary>
          ...
          {tabActiva === 'alertas' && (
            <div className="animate-fade-in flex-1 flex flex-col min-h-0 overflow-hidden">
              <PanelAlertas alertas={alertas} loading={loading} onAlertasChange={cargarAlertas} />
            </div>
          )}

          {tabActiva === 'etl' && (
            <div className="animate-fade-in flex-1 flex flex-col min-h-0 overflow-hidden" style={{ height: '100%' }}>
              <PanelETLLogs />
            </div>
          )}
